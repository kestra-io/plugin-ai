...
public class AIAgent extends Task implements RunnableTask<AIOutput>, OutputFilesInterface {
    ...
    public AbstractGraph graph(TaskRun taskRun, List<String> values, RelationType relationType) {
        ...
        nodes.add(new CustomGraphNode("provider", provider.getType()));
        if (this.tools != null) {
            int count = 0;
            for (ToolProvider tool : this.tools) {
                nodes.add(new CustomGraphNode("tool-" + count++, tool.getType()));
            }
        }
        if (this.contentRetrievers != null) {
            int count = 0;
            for (ContentRetrieverProvider contentRetriever : this.contentRetrievers) {
                nodes.add(new CustomGraphNode("contentRetriever-" + count++, contentRetriever.getType()));
            }
        }
        if (this.memory != null) {
            nodes.add(new CustomGraphNode("memory", memory.getType()));
        }
